...
from app.core import settings, ResourceNotFoundException, DomainException
...
class OCIStorageProvider(StorageProvider):
    """Implementação de armazenamento em nuvem Oracle Cloud (OCI Object Storage) via API compatível com S3."""

    def __init__(
        self,
        bucket_name: str | None = settings.OCI_BUCKET_NAME,
        region: str | None = settings.OCI_REGION,
        namespace: str | None = settings.OCI_NAMESPACE,
        access_key: str | None = settings.OCI_ACCESS_KEY_ID,
        secret_key: str | None = settings.OCI_SECRET_ACCESS_KEY,
        endpoint_url: str | None = settings.OCI_ENDPOINT_URL
    ):
        ...
        if not endpoint and self.namespace and self.region:
            endpoint = f"https://{self.namespace}.compat.objectstorage.{self.region}.oraclecloud.com"
    ...
    def save_upload_file(self, file_obj: BinaryIO, filename: str) -> str:
        """Salva o PDF no bucket OCI sob o prefixo uploads/."""
    ...
    def save_output_html(
        ...
        """Salva o HTML final gerado no bucket OCI sob o prefixo outputs/."""
    ...
    def get_html_download_info(self, html_filename: str) -> StorageDownloadInfo:
        ...
        try:
            ...
            logger.error(f"Erro ao consultar Oracle OCI: {e}")
            raise DomainException("Erro de configuração no Oracle Storage.", status_code=500)

    def prepare_local_pdf(self, pdf_key: str) -> str:
        """Baixa o PDF do bucket OCI para um arquivo temporário local e retorna o caminho."""
        try:
            ...
            logger.error(f"Erro ao baixar arquivo do Oracle OCI {pdf_key}: {e}")
            raise ResourceNotFoundException(f"Erro ao procurar PDF {pdf_key}: {e}")

    def exists(self, key: str) -> bool:
        """Verifica se uma chave existe no bucket OCI."""
        try:
            self.client.head_object(Bucket=self.bucket_name, Key=key)
            return True
        except ClientError:
